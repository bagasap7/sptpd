...
class PajakHiburanController extends Controller
{
    ...
    {
        return view('dashboard.pajak_hiburan.index',([
            'wajibpajak' => WajibPajak::all(),
            'objekpajak'=> ObjekPajak::where('id_jenis_pajak','=' ,'3') ->when(Auth::user()->id_wajib_pajak,function($query){
                $query->where('id_wajib_pajak',Auth::user()->id_wajib_pajak);
            }) ->get(),
            "transaksi" => Transaksi::where('id_jenis_pajak','=','3')->when(Auth::user()->id_wajib_pajak,function($query){
                $query->where('id_wajib_pajak',Auth::user()->id_wajib_pajak);
            })->get()
        ]));
    }
    public function create($id)
    {
        ...
        return view('dashboard.pajak_hiburan.create',compact([
            'objekpajak','q','kd'
        ]));
    }
    ...
    {   
        ...
        // dd($transaksi);
         ...
         return redirect()->route('pajak_hiburan.index')->with('success','Pajak Hiburan berhasil ditambahkan');
    }
}
